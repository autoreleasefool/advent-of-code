...
def find_right_edge(current_tile):
    ...
    while (
        in_range(right_tile)
        and right_tile not in clay
        and (under_tile in clay or under_tile in settled_water)
    ):
...
while not spilled_water or moving_water:
    ...
    if not right_enclosed:
        if in_range(under_right) and (
            not moving_water or moving_water[-1] != under_right
        ):
