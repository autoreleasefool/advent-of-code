...
class Session:
  ...
  def __init__(self, token: Optional[str], language: Optional[Language], year: Optional[int], day: Optional[int]):
    self.token = token
    ...
    if path.exists(_cache_file):
      with open(_cache_file) as f:
        ...
        self.challenge = Challenge(
          year if year else cache['year'],
          day if day else cache['day']
        )
  ...
  def working_directory(self):
    return path.join(
      self.challenge.working_directory,
      self.language.value
    )
