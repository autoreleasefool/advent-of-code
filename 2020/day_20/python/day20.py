...
def operations_to_fit_tile(tile, left_fit, top_fit):
    ...
    if left_fit == left and top_fit == top:
        return ()
    if left_fit == left[::-1] and top_fit == bottom:
        return "v"
    if left_fit == top and top_fit == left:
        return ("r", "v")
    if left_fit == top[::-1] and top_fit == right:
        return "r"
    if left_fit == right and top_fit == top[::-1]:
        return "h"
    if left_fit == right[::-1] and top_fit == bottom[::-1]:
        return ("v", "h")
    if left_fit == bottom and top_fit == left[::-1]:
        return ("r", "v", "h")
    if left_fit == bottom[::-1] and top_fit == right[::-1]:
        return ("r", "h")
    return None
...
def fit_next_tile(grid, remaining_tiles, pos):
    ...
    for t in remaining_tiles:
        ...
        fits = fit_next_tile(
            grid[:] + [t],
            remaining_tiles - set([t]),
            Position(pos.x + 1, pos.y) if pos.x < width - 1 else Position(0, pos.y + 1),
        )
        if fits:
            return fits
...
def solve_grid():
    for t in tiles:
        for ops in ops_orientations:
            ...
            solved_grid = fit_next_tile(
                [t], set(tiles.keys()) - set([t]), Position(1, 0)
            )
            if solved_grid:
                return solved_grid
...
aoc.p1(
    solved_grid[0]
    * solved_grid[width - 1]
    * solved_grid[(height - 1) * width]
    * solved_grid[(height - 1) * width + width - 1]
)
...
def mark_sea_monster(grid, x, y):
    for xd in range(sea_monster_width):
        for yd in range(sea_monster_height):
            if (
                sea_monster[yd][xd] != "-"
                and grid[y + yd][x + xd] != sea_monster[yd][xd]
            ):
                return False
    for xd in range(sea_monster_width):
        for yd in range(sea_monster_height):
