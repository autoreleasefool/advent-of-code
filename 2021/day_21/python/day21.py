...
def possibilities(scores, positions, current_player):
    ...
    for r1, r2, r3 in product(range(1, 4), range(1, 4), range(1, 4)):
        ...
        next_positions = (new_position, positions[1]) if current_player == 0 else (positions[0], new_position)

        new_score = scores[current_player] + new_position + 1
        next_score = (new_score, scores[1]) if current_player == 0 else (scores[0], new_score)
